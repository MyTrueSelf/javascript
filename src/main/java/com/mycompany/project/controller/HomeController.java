...
public class HomeController {
	...
	public void ajaxLogin(String mid,String mpassword, HttpSession httpSession ,HttpServletResponse response) throws Exception{
		LOGGER.info("실행");
		...
		if(db.containsKey(mid)) {
			if(db.get(mid).equals(mpassword)) {
				result = "success";
				httpSession.setAttribute("sessionMid", mid);
			}else {
		...
		response.setContentType("applicatoion/json; charset=UTF-8");
		JSONObject jsonObject = new JSONObject();
	...
	public void ajaxLogout(HttpSession httpSession ,HttpServletResponse response) throws Exception{
		LOGGER.info("실행");
